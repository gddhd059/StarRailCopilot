    ...
    def rogue_path_select(self, path: str | RoguePath, skip_first_screenshot=True):
        ...
        while 1:
            ...
            if self.match_template_color(ROGUE_LAUNCH, interval=2):
                if not self.image_color_count(ROGUE_LAUNCH, color=(223, 223, 225), threshold=15, count=50):
            ...
            if self.interval_is_reached(entry, interval=2) and self._is_page_rogue_path():
                if not self.image_color_count(PATH_LOADED_CHECK, color=(246, 246, 246), threshold=15, count=50):
            ...
            if self.appear(CONFIRM_PATH, interval=2):
                if not self.image_color_count(CONFIRM_PATH, color=(223, 223, 225), threshold=15, count=50):
